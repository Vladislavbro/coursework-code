...
import random
from config import NUM_TEXTS_TO_ANNOTATE, BATCH_SIZE, TRAIN_FILE_PATH, TEST_FILE_PATH, DATA_DIR

def get_prompt_string(num_shots):
    random.seed(42)
    all_examples = load_mrc(TRAIN_FILE_PATH)
    example_json = random.sample(all_examples, num_shots) if num_shots > 0 else []
    all_texts = load_mrc(TEST_FILE_PATH)
    texts_to_annotate = [item["text"] for item in all_texts[:NUM_TEXTS_TO_ANNOTATE]]
    
    # Загружаем трудные примеры для демонстрации правильной разметки
    difficult_examples = load_mrc(DATA_DIR / "difficult_examples.json")
...
Обрати внимание на эти сложные примеры правильной разметки:
...
def get_batch_prompt_string(num_shots, batch_texts):
    random.seed(42)
    all_examples = load_mrc(TRAIN_FILE_PATH)
    example_json = random.sample(all_examples, num_shots) if num_shots > 0 else []
